angular.module('classeur.opt.filePropertiesDialog', [])
		...
		return {
			...
			link: function(scope) {
				...
				scope.$watch('editorLayoutSvc.currentControl === "filePropertiesDialog"', function(show) {
					...
					$mdDialog.show({
						templateUrl: 'opt/filePropertiesDialog/filePropertiesDialog.html',
						controller: function(scope) {
							var properties = fileDao.contentDao.properties || {};
							scope.properties = Object.keys(properties).sort().map(function(key) {
								return {
									key: key,
